...
- Some people have seen issues with FFB sticks sometimes not properly using FFB effects or getting stuck, even using the FFB-specific firmware.  I've been able to replicate the issue both on this adaptor and on one which uses the original Teensy 2.0-based design, so in theory, whatever the issue is may be able to be resolved if somebody updates the firmware.  Given that it impacts certain users but not others, it seems likely to be specific to particular revisions of the FFB stick.
